...
function findWordOccurrences(str, word) {
  ...
  // Створюємо пустий масив matches.
  let matches = [];

  // Змінна match без значення.
  let match;

  // За допомогою циклу while створюємо ітерацію, поки рядок містить збіги з регулярним виразом.
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  // Заміняємо всі входження слова у рядку на нове слово.
  let replacedString = str.replace(regex, newWord);
...
function checkFlags(regex) {
  ...
  if (regex.global) {
    flagsUsed.push("global");
  }

  // Перевіряємо, чи використовується флаг 'm' (multiline) у регулярному виразі.
  if (regex.multiline) {
    flagsUsed.push("multiline");
  }

  // Перевіряємо, чи використовується флаг 's' (dotAll) у регулярному виразі.
  if (regex.dotAll) {
    flagsUsed.push("dotAll");
  }

  // Перевіряємо, чи використовується флаг 'y' (sticky) у регулярному виразі.
  if (regex.sticky) {
    flagsUsed.push("sticky");
  }
